...
counts <- read.csv(paste0(indir,"microglia_with_gender_effect/counts_per_sample_per_microglia_subcluster.csv"), header = T)
pheno <- counts %>%
...
estimateCellStateChange <- function(k, counts, histopheno) {
  ...
  colnames(cluster_counts)[10:17] <- c("Hippocampus_Vol_mm3", "prop_AT8_Coverage_Area", "prop_GFAP_Coverage_Area", "prop_IBA1_Coverage_Area", "prop_CD68_Coverage_Area", "prop_S100B_Coverage_Area", "prop_MBP_Coverage_Area","HMGB1_Intensity_Cytoplasm")
...
write.csv(t(ClusterRes), file = paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_cluster_microglia_with_gender_effect.csv"))
...
histo_all<-read.csv(paste0(outdir,"log_odds_ratio_new_method_per_unit_per_histopathology_per_cluster_microglia_with_gender_effect.csv"))
...
pdf(paste0(plotdir,"microglia_with_gender_effect_subclusters_pathology_heatmap.pdf"), height=6, width=20)
